...
def _find_ramic_bridge_files() -> tuple[Path, Path]:
    """Locate the bundled RAMIC Bridge daemon script and SKILL IL file."""
    try:
        resources = importlib.resources.files("virtuoso_bridge.virtuoso.basic.resources")
        daemon_ref = resources / "ramic_bridge_daemon_27.py"
        il_ref = resources / "ramic_bridge.il"
        # Materialize to real paths (works for both installed and editable installs)
        with (
            importlib.resources.as_file(daemon_ref) as daemon_path,
            importlib.resources.as_file(il_ref) as il_path,
        ):
            if daemon_path.is_file() and il_path.is_file():
                return daemon_path, il_path
    except (ModuleNotFoundError, FileNotFoundError, TypeError):
        pass

    raise FileNotFoundError(
        "Cannot locate ramic_bridge_daemon_27.py and ramic_bridge.il in "
...
def _generate_virtuoso_setup_il(daemon_path: str, il_path: str) -> str:
...
class RAMICBridge(VirtuosoInterface):
    ...
    def ensure_remote_setup(self) -> None:
        ...
        daemon_local, il_local = _find_ramic_bridge_files()
        ...
        remote_daemon = f"{self._remote_work_dir}/ramic_bridge_daemon_27.py"
        ...
        logger.info("Uploading daemon script to %s", remote_daemon)
        ...
        setup_content = _generate_virtuoso_setup_il(remote_daemon, remote_il)
        ...
        logger.info("Remote setup complete; setup script at %s", remote_setup)
